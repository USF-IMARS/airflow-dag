...
download_granule = BashOperator(
    ...
    bash_command="""
        METADATA_FILE={{ params.filepather.metadata(execution_date, dag_run.conf['region']['place_name']) }} &&
        OUT_PATH={{ params.filepather.myd01(execution_date, dag_run.conf['region']['place_name']) }}         &&
...
l1a_2_geo = BashOperator(
    ...
    bash_command="""
        ...
        --output={{params.geo_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        {{params.l1a_pather(execution_date, dag_run.conf['region']['place_name'])}}
    """,
    params={
...
make_l1b = BashOperator(
    ...
    bash_command="""
        ...
        --okm={{params.okm_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        --hkm={{params.hkm_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        --qkm={{params.qkm_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        {{params.l1a_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        {{params.geo_pather(execution_date, dag_run.conf['region']['place_name'])}}
    """,
    params={
        'l1a_pather': satfilename.myd01,
        'geo_pather': satfilename.l1a_geo,
...
l2gen = BashOperator(
    ...
    bash_command="""
        ...
        ifile={{params.l1b_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        ofile={{params.l2_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        geofile={{params.geo_pather(execution_date, dag_run.conf['region']['place_name'])}} \
        par={{params.parfile}}
    """,
    params={
