...
this_dag = DAG(
    ...
    default_args=get_default_args(
        start_date=datetime(2018, 5, 5, 5, 5)  # any date in past is fine
...
copy_file = IMaRSETLBashOperator(
    ...
    inputs={
        ...
        "my_input_file":
            ...
            # "Granule Date-matched"
            # (ie airflow execution_date == observation granule start date),
            # then you do not need to change this.
            "product_id="+str(CP_INPUT_PRODUCT_ID)+" AND date_time='{{ts}}'"
    },
    outputs={
        # Like in "inputs", you can change the name here so long as it matches
        # the bash_command.
        # The only parts of this you need to worry about are the CONSTANTS
        # which were set above.
        # ...Unless this DAG is not Granule-Date-Matched - in that case
        # the various times must be set in some other way.
        'my_output_file': {
            "product_id": CP_OUTPUT_PRODUCT_ID,
            # json, sql, & time set up metadata for the imars_product_metadata
            # db so that this outputs can be found by other scripts.
            "json": '{'
            ...
            "time": "{{ ts }}",
            # These modify details of how products are loaded.
            # For Basic usage just copy-paste or remove these lines.
            'duplicates_ok': False,
            'nohash': False,
            "verbose": 3,
        },
